...
def evaluate_nearby_float(
    ...
    *,
    ...
    result['target_sell_price_usd'] = target_sell
    result['estimated_fair_usd'] = target_sell  # backward-compatible alias
    result['valuation_source'] = source
    if target_sell is not None and buy_price_usd:
        result['resale_roi_pct'] = round((float(target_sell) * 0.98 / float(buy_price_usd) - 1) * 100, 1)
        result['gross_resale_pct'] = round((float(target_sell) / float(buy_price_usd) - 1) * 100, 1)
    ...
    if target_sell is not None and normal_cs:
        result['float_premium_pct'] = round((float(target_sell) / float(normal_cs) - 1) * 100, 1)
    else:
        result['float_premium_pct'] = None
    if buy_price_usd:
        result['break_even_sell_price_usd'] = round(float(buy_price_usd) / 0.98, 2)
        result['sell_price_for_10pct_roi_usd'] = round(float(buy_price_usd) * 1.10 / 0.98, 2)
        result['sell_price_for_20pct_roi_usd'] = round(float(buy_price_usd) * 1.20 / 0.98, 2)
    else:
